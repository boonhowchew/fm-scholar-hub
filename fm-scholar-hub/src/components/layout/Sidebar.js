...
import {
  ...
  LogOut, Settings, Users, X, ChevronRight
} from 'lucide-react'
...
const NavItem = ({ to, icon: Icon, label, badge }) => (
  <NavLink
    ...
    onMouseEnter={e => { if (!e.currentTarget.classList.contains('active')) { e.currentTarget.style.background = 'var(--bg-tertiary)'; e.currentTarget.style.color = 'var(--text-primary)' } }}
  ...
  >
    ...
    {badge > 0 && (
      <span style={{
        ...
        fontSize: 11, fontWeight: 700, padding: '1px 7px', minWidth: 20,
        textAlign: 'center'
      }}>{badge}</span>
    )}
  </NavLink>
...
export default function Sidebar({ isOpen, onClose, unreadNotifications = 0 }) {
    ...
    <>
      ...
      <aside style={{
        ...
        <div style={{
          ...
          <button className="btn-icon" onClick={onClose} style={{ display: 'none' }}>
            <X size={18} />
          </button>
        </div>
        ...
        <nav style={{ flex: 1, padding: '8px 8px', overflowY: 'auto' }}>
          ...
          <NavItem to="/announcements" icon={Megaphone} label="Announcements" badge={unreadNotifications} />
        ...
        <div style={{ padding: '12px 8px', borderTop: '1px solid var(--border)' }}>
          <button
            ...
            style={{
              ...
              fontSize: 14, width: '100%', border: 'none', cursor: 'pointer',
