...
function ThreadList({ onSelectCategory }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('forum-threads-realtime')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'forum_threads' }, () => fetchThreads(selectedCat))
      .subscribe()
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>
            Discuss clinical cases, research, exam prep and more
          </p>
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        {categories.map(c => (
          <button key={c.id} onClick={() => handleCatFilter(c)}
            style={{
              padding: '7px 16px', borderRadius: 999, fontSize: 13, fontWeight: 500,
        ...
        {threads.length === 0 ? (
          ...
          >
            <div style={{
              ...
              background: getRoleColor(t.profiles?.role),
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                ...
                {t.profiles?.role === 'academic_staff' && (
                  <span style={{ color: 'var(--upm-gold)', fontWeight: 600 }}> · Staff</span>
                )}
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" placeholder="Thread title" value={form.title}
                onChange={e => setForm(p => ({ ...p, title: e.target.value }))} />
            </div>
            <div className="form-group">
              <label className="form-label">Content *</label>
              <textarea className="form-textarea" placeholder="Write your post here..." style={{ minHeight: 160 }}
...
function ThreadDetail() {
  ...
  useEffect(() => {
    fetchThread()
    fetchReplies()
    supabase.from('forum_threads').update({ views: (thread?.views || 0) + 1 }).eq('id', id)
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
          ...
          <div>
            <p style={{ fontWeight: 600, fontSize: 14 }}>
              ...
              {thread.profiles?.role === 'academic_staff' && (
                <span style={{ color: 'var(--upm-gold)', fontWeight: 600, fontSize: 12 }}> · Academic Staff</span>
              )}
            </p>
            <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              {format(new Date(thread.created_at), 'd MMM yyyy, h:mm a')}
            </p>
          </div>
        </div>
        <div style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    <p style={{ fontWeight: 600, fontSize: 13 }}>
                      ...
                      {r.profiles?.role === 'academic_staff' && (
                        <span style={{ color: 'var(--upm-gold)', fontWeight: 600, fontSize: 11 }}> · Staff</span>
                      )}
                    </p>
                    <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
      ...
      {!thread.is_locked ? (
        <div className="card">
          ...
          <textarea
            ...
            placeholder="Share your thoughts, clinical experience, or feedback..."
        ...
        <div className="alert alert-info">
          <Lock size={15} /> This thread has been locked. No new replies can be added.
        </div>
      )}
    </div>
  )
...
export { ThreadList, ThreadDetail }
